package com.example.zimuquan.circleofletters.http.okhttp;
...
public class OkHttpUtils
{
    ...
    private void executeLoadCache(final RequestCall requestCall, Callback callback)
    {
        ...
        if(cacheCall != null) {
            requestCall.getCacheCall().enqueue(new okhttp3.Callback() {
                @Override
                public void onFailure(Call call, final IOException e) {
                    sendCacheFailResultCallback(call, e, finalCallback, id);
                }
                ...
                public void onResponse(final Call call, final Response response) {
                    try {
                        if (call.isCanceled()) {
                            sendCacheFailResultCallback(call, new IOException("Canceled!"), finalCallback, id);
                            return;
                        }

                        if (!finalCallback.validateReponse(response, id)) {
                            sendCacheFailResultCallback(call, new IOException("request failed , reponse's code is : " + response.code()), finalCallback, id);
                            return;
                        }
    ...
    public void sendCacheFailResultCallback(final Call call, final Exception e, final Callback callback, final int id)
    {
        if (callback == null) return;
        ...
        mPlatform.execute(new Runnable()
        {
            ...
            public void run()
            {
                callback.onNoCache(call,404,e.getMessage(),id);
            }
        });
    }
